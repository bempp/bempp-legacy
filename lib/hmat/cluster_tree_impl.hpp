...
ClusterTree<N>::ClusterTree(const Geometry &geometry, int minBlockSize)
    ...
    template <int N>
    const shared_ptr<ClusterTreeNode<N>> ClusterTree<N>::root() {
      return m_root;
    }
    ...
    ClusterTree<N>::initializeClusterTree(const Geometry &geometry) {
      ...
      return make_shared<ClusterTreeNode<N>>(
          ClusterTreeNodeData(indexRange, b));
    }
    ...
    ClusterTree<2>::splitClusterTreeByGeometry(const Geometry &geometry,
      ...
      splittingFun = [&dofPermutation, &geometry, minBlockSize, &splittingFun](
        ...
        if (indexSet.size() > minBlockSize) {

          IndexSetType firstIndexSet;
          IndexSetType secondIndexSet;

          auto boxes =
              clusterTreeNode->data().boundingBox.divideMaxDimension(.5);
          ...
          for (int hMatDof = indexRange[0]; hMatDof < indexRange[1];
               ++hMatDof) {
            dofPermutation.addDofIndexPair(indexSet[originalIndexCount],
                                           hMatDof);
            ++originalIndexCount;
          }
        }
    ...
    template <int N>
    std::size_t ClusterTree<N>::mapOriginalDofToHMatDof(
        std::size_t originalDofIndex) const {
