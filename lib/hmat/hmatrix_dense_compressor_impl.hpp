    ...
    shared_ptr<HMatrixData<ValueType>> &hMatrixData) const
{
  ...
  auto rowIndexRange = blockClusterTreeNode.data().
    rowClusterTreeNode->data().indexRange();

  auto columnIndexRange = blockClusterTreeNode.data().
    columnClusterTreeNode->data().indexRange();

  hMatrixData.reset(new HMatrixDenseData<ValueType>(
        numberOfRowIndices,
        numberOfColumnIndices));

  m_dataAccessor.computeMatrixBlock(rowIndexRange,
      columnIndexRange,blockClusterTreeNode,hMatrixData.A());
