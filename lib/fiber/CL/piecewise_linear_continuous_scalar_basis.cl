...
__kernel void clBasisfElement(MeshParam prm,
                              __global const ValueType *g_meshVtx,
                              __global const int *g_meshIdx, int elIdx,
                              __global const ValueType *g_localPoints,
                              int nPoints, int pointDim, int nDof,
  ...
  if (nDof == 1) {
    ...
    case 0:
      result[pt] =
          1.0 - g_localPoints[pt * pointDim] - g_localPoints[pt * pointDim + 1];
      break;
    case 1:
  ...
  // note 1: basis functions are assumed independent of global geometry (for
  // now)
...
__kernel void
clBasisfElements(MeshParam prm, __global const ValueType *g_meshVtx,
                 __global const int *g_meshIdx, __global const int *g_elIdx,
                 int nEls, __global const ValueType *g_localPoints, int nPoints,
                 int pointDim, int nDof, __global ValueType *result) {
 ...
 */
void devBasisEval(const ValueType *point, ValueType *data) {
...
__kernel void clBasisProd(__global const ValueType *g_p1,
                          __global const ValueType *g_p2, int np,
