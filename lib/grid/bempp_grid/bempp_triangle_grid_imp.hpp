...
inline TriangleGrid::TriangleGrid(const shared_ptr<DataContainer> &data)
    : m_data(data) {
...
typename TriangleGrid::Codim<cd>::template Partition<pitype>::LevelIterator
TriangleGrid::lbegin(int level) const {

  return typename TriangleGrid::Codim<cd>::template Partition<pitype>::
      LevelIterator(
          LevelIteratorImp<cd, pitype, const TriangleGrid>(m_data, level, 0));
}

template <int cd, Dune::PartitionIteratorType pitype>
typename TriangleGrid::Codim<cd>::template Partition<pitype>::LevelIterator
TriangleGrid::lend(int level) const {

  return typename TriangleGrid::Codim<cd>::template Partition<pitype>::
      LevelIterator(LevelIteratorImp<cd, pitype, const TriangleGrid>(
          m_data, level, m_data->numberOfEntities<cd>(level)));
}

template <int cd, Dune::PartitionIteratorType pitype>
typename TriangleGrid::Codim<cd>::template Partition<pitype>::LevelIterator
TriangleGrid::leafbegin() const {
...
inline const typename TriangleGrid::GridFamily::Traits::LevelIndexSet &
TriangleGrid::levelIndexSet(int level) const {

  return *m_levelIndexSet[level];
}

inline const typename TriangleGrid::GridFamily::Traits::LeafIndexSet &
TriangleGrid::leafIndexSet() const {

  return *m_levelIndexSet[m_data->levels() - 1];
}

inline const typename TriangleGrid::GridFamily::Traits::GlobalIdSet &
TriangleGrid::globalIdSet() const {

  return m_globalIdSet;
}

inline const typename TriangleGrid::GridFamily::Traits::LocalIdSet &
TriangleGrid::localIdSet() const {

  return m_localIdSet;
}

template <int cd>
TriangleGrid::GridFamily::Traits::LevelIndexSet::IndexType
TriangleGrid::entityIndex(
    const typename TriangleGrid::GridFamily::Traits::Codim<cd>::Entity
        &entity) {
...
inline int TriangleGrid::maxLevel() const { return m_data->levels() - 1; }
