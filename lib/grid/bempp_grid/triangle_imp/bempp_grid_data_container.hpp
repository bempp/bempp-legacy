...
public:
  ...
  template <int cd>
  using DuneGeometry =
      typename TriangleGrid::GridFamily::Traits::Codim<cd>::Geometry;
  ...
  template <int cd> int numberOfEntities(int level) const;

  template <int cd> DuneGeometry<cd> geometry(const Entity<cd> &entity);

  template <int cd> unsigned int id(const Entity<cd> &entity);

  const std::array<unsigned int, 3> &
  element2Edges(int level, unsigned int elementIndex) const;
  const std::vector<unsigned int> &edge2Elements(int level,
                                                 unsigned int edgeIndex) const;
  const std::vector<unsigned int> &node2Elements(int level,
                                                 unsigned int nodeIndex) const;
  const std::vector<unsigned int> &node2Edges(int level,
                                              unsigned int nodeIndex) const;
  ...
  getElementSons(int level, unsigned int elementIndex) const;

private:
  template <int cd> void computeGeometries(int level);
  ...
  std::tuple<std::vector<shared_ptr<std::vector<Geometry<0>>>>,
             std::vector<shared_ptr<std::vector<Geometry<1>>>>,
             std::vector<shared_ptr<std::vector<Geometry<2>>>>> m_geometries;
};
}
