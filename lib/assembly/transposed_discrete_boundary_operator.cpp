...
TransposedDiscreteBoundaryOperator<ValueType>::TransposedDiscreteBoundaryOperator(
        TranspositionMode trans, const shared_ptr<const Base>& op) :
    m_trans(trans), m_operator(op)
{
    if (!m_operator.get())
        throw std::invalid_argument(
            "TransposedDiscreteBoundaryOperator::TransposedDiscreteBoundaryOperator(): "
            "the wrapped operator must not be NULL");
    if (m_trans != NO_TRANSPOSE && m_trans != TRANSPOSE &&
        m_trans != CONJUGATE && m_trans != CONJUGATE_TRANSPOSE)
...
template <typename ValueType>
arma::Mat<ValueType> TransposedDiscreteBoundaryOperator<ValueType>::asMatrix() const
{
...
template <typename ValueType>
unsigned int TransposedDiscreteBoundaryOperator<ValueType>::columnCount() const
{
        ...
        const ValueType beta) const
{
    // Bitwise xor. We use the fact that bit 0 of M_trans denotes
    // conjugation, and bit 1 -- transposition.
    m_operator->apply(TranspositionMode(trans ^ m_trans), x_in, y_inout,
                      alpha, beta);
}
