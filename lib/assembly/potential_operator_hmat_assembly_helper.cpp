...
void PotentialOperatorHMatAssemblyHelper<BasisFunctionType, ResultType>::
    dofVolumes(Vector<double>& testVolumes, Vector<double>& trialVolumes) const
{
        ...
        Matrix<ResultType>& data) const
{
    ...
    shared_ptr<const ComponentLists> componentLists = m_componentListsCache->get(testIndexRange[0], numberOfTestIndices);
    ...
    const std::vector<std::vector<int> >& componentIndices = componentLists->componentIndices;
    ...
    if (numberOfTrialIndices == 1) {
            ...
            for (size_t nTrialDof = 0; nTrialDof < trialLocalDofs[nTrialElem].size();
                ...
                for (size_t nPoint = 0; nPoint < pointIndices.size(); ++nPoint)
                    for (size_t nComponent = 0;
                        ...
                        data(blockRows[nPoint][nComponent], 0) += activeTrialLocalDofWeight * localResult[nPoint](componentIndices[nPoint][nComponent], 0);
            }
        }
        ...
        for (size_t nTrialElem = 0; nTrialElem < trialElementIndices.size();
            ...
            for (size_t nTrialDof = 0; nTrialDof < trialLocalDofs[nTrialElem].size();
                 ++nTrialDof)
                data(0, blockCols[nTrialElem][nTrialDof]) += trialLocalDofWeights[nTrialElem][nTrialDof] * localResult[nTrialElem](0, trialLocalDofs[nTrialElem][nTrialDof]);
    } else { // a "fat" block
                ...
                for (size_t nPoint = 0; nPoint < pointIndices.size(); ++nPoint)
                    for (size_t nComponent = 0;
                        ...
                        data(blockRows[nPoint][nComponent],
                            ...
                            += trialLocalDofWeights[nTrialElem][nTrialDof] * localResult(nPoint,
