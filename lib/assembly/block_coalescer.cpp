...
void getLeafBlclusters(blcluster* bc, blcluster** leafClusters, unsigned leafCount)
{
    if (!bc)
...
BlockCoalescer<ValueType>::BlockCoalescer(
    ...
    m_blocks(blocks),
    m_decomposedBlocks(decomposedBlocks),
    ...
    for (unsigned j = 0; j < decomposedColCount; ++j, ++colIndex) {
        if (m_permutedTrialGlobalToFlatLocalMap)
            m_permutedTrialGlobalToFlatLocalMap->ExtractMyRowView(
                        dbc->getb2() + j,
                        trialEntryCount, trialValues, trialIndices);
        else {
            trialEntryCount = 1;
            trialValues = &ONE;
        ...
        for (int trialDof = 0; trialDof < trialEntryCount; ++trialDof) {
            ...
            if (n >= colCount)
                throw std::runtime_error(
                        "BlockCoalescer::coalesceDenseBlock(): "
            ...
            for (unsigned i = 0; i < decomposedRowCount; ++i, ++rowIndex) {
                if (m_permutedTestGlobalToFlatLocalMap)
                    m_permutedTestGlobalToFlatLocalMap->ExtractMyRowView(
                                dbc->getb1() + i,
                                testEntryCount, testValues, testIndices);
                else {
                    testEntryCount = 1;
                    testValues = &ONE;
                ...
                for (int testDof = 0; testDof < testEntryCount; ++testDof) {
                    ...
                    if (m >= rowCount)
                        throw std::runtime_error(
                                ...
                                "block is not independent");
                    data[m + n * rowCount] +=
                            testValues[testDof] * trialValues[trialDof] *
    ...
    if (m_permutedTestGlobalToFlatLocalMap) {
        ...
        for (unsigned i = 0; i < decomposedRowCount; ++i) {
            ...
            for (int testDof = 0; testDof < testEntryCount; ++testDof) {
                ...
                if (m >= rowCount)
                    throw std::runtime_error(
                            "BlockCoalescer::coalesceLowRankBlock(): "
                            "block is not independent");
                for (int k = 0; k < rank; ++k) {
                    ...
                    U[m + k * rowCount] +=
                            testValues[testDof] *
                            decomposedData[i + k * decomposedRowCount];
                }
            }
        }
    } else
            ...
            for (int trialDof = 0; trialDof < trialEntryCount; ++trialDof) {
                ...
                for (int k = 0; k < rank; ++k)
                    V[n + k * colCount] +=
                            ...
                            decomposedData[rank * decomposedRowCount +
                                           j + k * decomposedColCount];
            }
        }
    } else
        V = decomposedData + rank * decomposedRowCount;

    m_blocks[index]->cpyLrM_cmpr(
                rank, U, rowCount, V, colCount, m_acaOptions.eps, rank);
    delete m_decomposedBlocks[index];
    m_decomposedBlocks[index] = 0;
}
