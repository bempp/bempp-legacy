...
cdef extern from "bempp_ext/operators/far_field/py_far_field_operators.hpp" namespace "Bempp":
...
def single_layer(Space space not None,
        ...
        op.impl_.assign(
             helmholtz_single_layer_far_field_discrete_operator(
...
def double_layer(Space space not None,
        ...
        op.impl_.assign(
             helmholtz_double_layer_far_field_discrete_operator(
                 space.impl_,np_to_eigen_matrix_float64(points),
                 complex_double(_np.real(wave_number), _np.imag(wave_number)),
                 deref(parameters.impl_)))
        return op
