...
def prism(origin=(0,0,0), length=1, sides=3, radius=1, direction=(0,0,1), polygon_orientation=(1,0,0), h=0.1, force_perpendicular=True):
    """
    ...
    if np.linalg.norm(np.cross(direction,polygon_orientation))==0:
        raise ValueError("The orientation of the polygon and the direction of the prism cannot be parallel")


    origin = np.array(origin)
    direction = np.array(direction) / np.sqrt(sum(i**2 for i in direction))
    polygon_orientation = np.array(polygon_orientation) / np.sqrt(sum(i**2 for i in polygon_orientation))
    perp = np.cross(polygon_orientation, direction)

    if force_perpendicular:
        polygon_orientation = polygon_orientation - np.dot(polygon_orientation, direction) * direction

    prism_stub = ""
    for i in range(sides):
        point = origin + radius * polygon_orientation * np.cos(i/sides * 2*np.pi) + radius * perp * np.sin(i/sides * 2*np.pi)
        prism_stub += "Point("+str(i+1)+") = {"+(",".join(str(p) for p in point))+",cl};"
        prism_stub += "\n"
        point += length * direction
        prism_stub += "Point("+str(sides+i+1)+") = {"+(",".join(str(p) for p in point))+",cl};"
        prism_stub += "\n"
    prism_stub += "\n"

    for i in range(sides):
        prism_stub += "Line("+str(i+1)+") = {"+str(i+1)+","+str((i+1)%sides+1)+"};"
        prism_stub += "\n"
        prism_stub += "Line("+str(sides+i+1)+") = {"+str(i+1)+","+str(i+sides+1)+"};"
        prism_stub += "\n"
        prism_stub += "Line("+str(2*sides+i+1)+") = {"+str(i+sides+1)+","+str((i+1)%sides+sides+1)+"};"
        prism_stub += "\n"
    prism_stub += "\n"

    prism_stub += "Line Loop(1) = {"+(",".join(str(i+1) for i in range(sides)))+"};"
    prism_stub += "\n"
    prism_stub += "Line Loop(2) = {"+(",".join(str(-i-2*sides-1) for i in range(sides)))+"};"
    prism_stub += "\n"
    for i in range(sides):
        prism_stub += "Line Loop("+str(i+3)+") = {"+str(-i-1)+","+str(sides+i+1)+","+str(str(i+2*sides+1))+","+str(-((i+1)%sides)-sides-1)+"};"
        prism_stub += "\n"
    prism_stub += "\n"

    for i in range(1,sides+3):
        prism_stub += "Plane Surface("+str(i)+") = {"+str(i)+"};"
        prism_stub += "\n"
    prism_stub += "\n"

    for i in range(1,sides+3):
        prism_stub += "Physical Surface("+str(i)+") = {"+str(i)+"};"
        prism_stub += "\n"
    prism_stub += "\n"

    prism_stub += "Surface Loop(1) = {"+(",".join(str(i) for i in range(1,sides+3)))+"};"
    prism_stub += "\n"
    prism_stub += "Volume(1) = {1};"
    prism_stub += "\n"
    prism_stub += "Mesh.Algorithm = 6;"
    prism_geometry = "cl = " + str(h) + ";\n" + prism_stub

    with open("/home/mscroggs/python/bempp-test/prism.geo","w") as f:
        f.write(prism_geometry)

    return __generate_grid_from_geo_string(prism_geometry)
