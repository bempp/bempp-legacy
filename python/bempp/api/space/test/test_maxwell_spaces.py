...
class TestMaxwellSpaces(TestCase):
    ...
    def test_rwg_space_is_correctly_scaled(self):
        ...
        for element in self._grid.leaf_view.entity_iterator(0):
            dofs_rt, weights_rt = space_rt.get_global_dofs(element, dof_weights=True)
            dofs_rwg, weights_rwg = space_rwg.get_global_dofs(element, dof_weights=True)
    ...
    def test_maxwell_barycentric_rwg_mass_agrees_with_rwg_mass(self):
        ...
        ident_rwg = bempp.api.operators.boundary.sparse.identity(space_rwg, space_rwg, space_rwg)
        ident_rwg_b = bempp.api.operators.boundary.sparse.identity(space_rwg_b, space_rwg_b, space_rwg_b)
