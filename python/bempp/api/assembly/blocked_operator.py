    ...
    def __init__(self, op1, op2):
        ...
        for index in range(op1.ndims[0]):
            if op1.range_spaces[index] != op2.range_spaces[index]:
                raise ValueError(
                    "Range spaces at index {0} are not identical.".format(
                        index))

        for index in range(op1.ndims[0]):
            if op1.dual_to_range_spaces[index] != \
                    op2.dual_to_range_spaces[index]:
                raise ValueError(
        ...
        for index in range(op1.ndims[1]):
            if op1.domain_spaces[index] != op2.domain_spaces[index]:
                raise ValueError(
                    "Domain spaces at index {0} are not identical.".format(
                        index))
...
class ProductBlockedOperator(BlockedOperatorBase):
    ...
    def __init__(self, op1, op2):
        ...
        super(ProductBlockedOperator, self).__init__(
...
class BlockedDiscreteOperatorBase(DiscreteBoundaryOperator):
    ...
    def __rmul__(self, other):
        ...
        if _np.isscalar(other):
            ...
            raise ValueError(
                "Cannot multiply operand of type {0} from the left.".format(
                    type(other)))
