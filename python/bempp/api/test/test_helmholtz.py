...
class TestHelmholtz(TestCase):
    ...
    def test_operator_preconditioned_dirichlet_solve_dual(self):
        ...
        dlp = bempp.api.operators.boundary.helmholtz.double_layer(lin_space, lin_space, const_space, k)
        ...
        sol_grid_fun, info, res = bempp.api.linalg.gmres(hyp * slp, rhs, return_residuals=True)

        rel_error = sol_grid_fun.relative_error(sol_fun)
        self.assertTrue(rel_error < 2E-2, msg="Actual error: {0}. Expected error: 1E-2".format(rel_error))
        self.assertTrue(len(res) < 9, msg="Needed {0} iterations to solve system. Expected not more than 8 iterations.".format(len(res)))
    ...
    def test_operator_preconditioned_dirichlet_solve_lin(self):
        ...
        rel_error = sol_grid_fun.relative_error(sol_fun)
        self.assertTrue(rel_error < 2E-2, msg="Actual error: {0}. Expected error: 1E-2".format(rel_error))
        self.assertTrue(len(res) < 10, msg="Needed {0} iterations to solve system. Expected not more than 9 iterations.".format(len(res)))
	...
	def neumann_data(x, n, domain_index, res):
        ...
        sl = bempp.api.operators.potential.helmholtz.single_layer(space, point, 1)
        dl = bempp.api.operators.potential.helmholtz.double_layer(space, point, 1)
        ...
        self.assertTrue(rel_error  < 1E-6, msg="Actual error: {0}. Expected error: 1E-2".format(rel_error))
