...
class TestLaplace(TestCase):
    ...
    def test_solve_low_order_laplace_dirichlet_problem_on_sphere(self):
        ...
        self.assertTrue(rel_error < 1E-2, msg="Actual error: {0}. Expected error: 1E-2".format(rel_error))
	...
	def neumann_data(x, n, domain_index, res):
	    grad = np.array([np.exp(x[0] ) * np.sin(x[1]), np.exp(x[0]) * np.cos(x[1]), 0])
	    res[0] = np.dot(grad, n)
    ...
    def test_operator_preconditioned_dirichlet_solve_dual(self):
        ...
        dlp = bempp.api.operators.boundary.laplace.double_layer(lin_space, lin_space, const_space)
        ...
        sol_grid_fun, info, res = bempp.api.linalg.cg(hyp * slp, rhs, return_residuals=True)
