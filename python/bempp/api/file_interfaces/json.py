...
def export_data_sets(file_name, grid_data_sets, description=''):
    ...
    output = {'timestamp': timestamp(),
              ...
              'grid_data_sets': [dataset.as_dict() 
                  for dataset in grid_data_sets]}

    with open(file_name, 'wb') as outfile:
        json.dump(output, codecs.getwriter('utf-8')(outfile), 
...
def import_data_sets(file_name=None, data_string=None):
    ...
    data_dict['grid_data_sets'] = [GridDataSet.from_dict(dataset) 
            for dataset in data_dict['grid_data_sets']]
...
def export(file_name, **kwargs):
    ...
    """
    import bempp.api
    from bempp.api.file_interfaces.general_interface import bempp_object_to_grid_data_set
