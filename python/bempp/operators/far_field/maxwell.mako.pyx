...
cdef extern from "bempp/operators/py_operators.hpp" namespace "Bempp":
    cdef shared_ptr[const c_DiscreteBoundaryOperator[complex_double]] py_maxwell_single_layer_far_field_discrete_operator(
                const SpaceVariants& space,
                const Mat[double]& evaluationPoints,
                complex_double waveNumber,
                const c_ParameterList& parameterList) 

    cdef shared_ptr[const c_DiscreteBoundaryOperator[complex_double]] py_maxwell_double_layer_far_field_discrete_operator(
                const SpaceVariants& space,
                const Mat[double]& evaluationPoints,
                complex_double waveNumber,
                const c_ParameterList& parameterList) 
