...
class BlockedLinearOperator(object):
    ...
    def weak_form(self):
        ...
        discrete_operator = BlockedDiscreteLinearOperator(self._m,self._n)
...
class BlockedDiscreteLinearOperator(object):
    ...
    def __mul__(self,x):
        ...
        if _np.isscalar(x):
            res = BlockedDiscreteLinearOperator(self.ndims[0],self.ndims[1])
    ...
    def as_matrix(self):
        ...
        the_matrix = _lil((int(self._rows.sum()),int(self._cols.sum())),dtype=self.dtype)
        for i in range(self._m):
            for j in range(self._n):
                if self[i,j] is not None:
                    try:
                        part = self[i,j].as_matrix()
                    except:
                        part = self[i,j]
                    the_matrix[
